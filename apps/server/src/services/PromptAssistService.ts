...
export class PromptAssistService {
  async assist(input: {
    ...
    const prevAgentList = (input.previousAgents ?? []).map((a: any) => `{agents.${a.uid}.output}`).join(', ')
...
Available prior agent outputs: ${prevAgentList || 'none'}
...
Reference variables using {key} syntax. Reference prior agent outputs using {agents.uid.output} syntax.
